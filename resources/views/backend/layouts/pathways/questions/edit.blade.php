        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Question Edit</h4>
                    <a href="{{ route('admin.pathway-questions.index') }}" class="btn btn-sm btn-primary">Back</a>
                </div>

                <form action="{{ route('admin.pathway-questions.update', $question->id) }}" method="POST" id="questionForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="row gy-4">
                            ...
                            <div class="col-xxl-6 col-md-6">
                                ...
                                <select class="form-select @error('step_number') is-invalid @enderror" name="step_number" id="step_number" required>
                                    <option value="" disabled>Choose Step Level</option>
                                    <option value="1" {{ old('step_number', $question->step_number) == 1 ? 'selected' : '' }}>Step 1 (First Question)</option>
                                    <option value="2" {{ old('step_number', $question->step_number) == 2 ? 'selected' : '' }}>Step 2 (Middle Question)</option>
                                    <option value="3" {{ old('step_number', $question->step_number) == 3 ? 'selected' : '' }}>Step 3 (Final Question)</option>
                                </select>
                                @error('step_number')
                            ...
                            <div class="col-xxl-6 col-md-6">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" name="status" id="statusSelect">
                                    <option value="1" {{ old('status', $question->status) == 1 ? 'selected' : '' }}>Published</option>
                                    <option value="0" {{ old('status', $question->status) == 0 ? 'selected' : '' }}>Unpublished</option>
                                </select>
                                @error('status')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-xxl-12 col-md-12">
                                <div>
                                    ...
                                    <input type="text" name="question_text" id="question_text" class="form-control @error('question_text') is-invalid @enderror" placeholder="e.g. Which best reflects your role or organisation?" value="{{ old('question_text', $question->question_text) }}" required>
                                    @error('question_text')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <table class="table table-bordered table-centered" id="optionsTable">
                                    ...
                                    <tbody id="optionsContainer">
                                         @foreach($question->options as $index => $option)
                                             <tr class="option-row">
                                                 <td>
                                                     <input type="text" name="options[{{ $index }}][option_text]" class="form-control" placeholder="Enter option text" value="{{ old('options.'.$index.'.option_text', $option->option_text) }}" required>
                                                 </td>
                                                 <td>
                                                     <select name="options[{{ $index }}][target_type]" class="form-select target-type-select" required>
                                                         <option value="" disabled {{ $option->next_question_id == null && $option->result_id == null ? 'selected' : '' }}>Select Target Type</option>
                                                         <option value="next_question" {{ $option->next_question_id != null ? 'selected' : '' }}>Next Question</option>
                                                         <option value="result" {{ $option->result_id != null ? 'selected' : '' }}>Final Result</option>
                                                     </select>
                                                 </td>
                                                 <td>
                                                     <div class="target-question-container {{ $option->next_question_id != null ? '' : 'd-none' }}">
                                                         <select name="options[{{ $index }}][next_question_id]" class="form-select" {{ $option->next_question_id != null ? 'required' : '' }}>
                                                             <option value="" disabled {{ $option->next_question_id == null ? 'selected' : '' }}>Select Next Question</option>
                                                             @foreach($questions as $q)
                                                                 <option value="{{ $q->id }}" data-step="{{ $q->step_number }}" {{ $option->next_question_id == $q->id ? 'selected' : '' }}>Step {{ $q->step_number }}: {{ Str::limit($q->question_text, 40) }}</option>
                                                             @endforeach
                                                         </select>
                                                     </div>
                                                     <div class="target-result-container {{ $option->result_id != null ? '' : 'd-none' }}">
                                                         <select name="options[{{ $index }}][result_id]" class="form-select" {{ $option->result_id != null ? 'required' : '' }}>
                                                             <option value="" disabled {{ $option->result_id == null ? 'selected' : '' }}>Select Result</option>
                                                             @foreach($results as $r)
                                                                 <option value="{{ $r->id }}" {{ $option->result_id == $r->id ? 'selected' : '' }}>{{ $r->title }}</option>
                                                             @endforeach
                                                         </select>
                                                     </div>
                                                 </td>
                            ...
                            <div class="col-xxl-12 col-md-12">
                                <button type="submit" class="btn btn-primary">Update Question & Flow</button>
                            </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            // Start index from the count of pre-rendered options
            let index = {{ $question->options->count() }};
            ...
            // Apply restrictions initially on page load
            applyStepRestrictions();
